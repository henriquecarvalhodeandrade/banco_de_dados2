...
def realizar_emprestimo(usuario, exemplar, session):
    ...
    # 1. CHECAGENS DE REGRAS DE NEGÓCIO (Pré-transação)
    if exemplar.disponivel == 0:
        return False, "O exemplar não está disponível para empréstimo."
    
    # O 'pode_emprestar' já atualiza multas e faz a checagem de limite
    pode, mensagem = pode_emprestar(usuario, session) 
    if not pode:
        # Se falhar a regra de negócio, retorna ANTES de iniciar a transação
        return False, mensagem
        
    # 2. PREPARAÇÃO DA TRANSAÇÃO (Staging)
    
    # Ação 1: Cria o registro de Empréstimo
    novo_emprestimo = Emprestimo(usuario=usuario, exemplar=exemplar, data_emprestimo=date.today())
    
    # Ação 2: Atualiza o status do Exemplar para indisponível
    exemplar.disponivel = 0
    
    # Adiciona as mudanças à sessão (incluindo quaisquer multas atualizadas por 'pode_emprestar')
    session.add(novo_emprestimo)
    session.add(exemplar)

    # 3. CONTROLE TRANSACIONAL (COMMIT ou ROLLBACK)
    try:
        session.commit() 
        return True, f"Empréstimo de '{exemplar.material.titulo}' realizado com sucesso!"
        
    except Exception as e:
        session.rollback() # <--- Ponto Crítico: Desfaz TODAS as alterações.
        # Retorna a mensagem de erro detalhada
        return False, f"Erro crítico ao realizar empréstimo. Transação desfeita: {str(e)}"
